...
class StackLogHandler(logging.Handler):
    def __init__(self, key, bucket_id, options={}):
    ...
    def send_request(self):
        ...
        self.secondary = []
        if self.buf[0]['level'] == "INFO":
            log_level_id = 1
        elif self.buf[0]['level'] == "WARNING":
            log_level_id = 2
        elif self.buf[0]['level'] == "DEBUG":
            log_level_id = 3
        elif self.buf[0]['level'] == "ERROR":
            log_level_id = 4
        elif self.buf[0]['level'] == "CRITICAL":
            log_level_id = 5
        else:
            log_level_id = 1
        data = {
            'logMessage': self.buf[0]['line'],
            'stackKey': "FvVQcNHc8",
            'logTypeId': log_level_id
        }
        try:
            res = requests.post(
               url=self.url + self.bucket_id,
               json=data,
               stream=True,
